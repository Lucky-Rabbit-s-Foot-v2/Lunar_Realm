 ...
 * - StageID 기준으로 해당 스테이지 소환 후보 EnemyID/가중치, 스폰 주기, 보상 정보를 제공
 *
 * NOTE(260208, Codex):
 * - 요구사항 확정 전이므로 최소 필수 필드 + 확장 필드를 함께 정의함.
 * - StageID 관리 주체는 GameInstance를 가정하며, Spawner는 해당 ID로 데이터 조회함.
 */
// =============================================================================
USTRUCT(BlueprintType)
struct FStageStaticData : public FTableRowBase
{
	...
	// SpawnEnemyIDs와 같은 인덱스의 확률 가중치(합 1.0 권장)
	UPROPERTY(EditAnywhere, BlueprintReadWrite, Category = "LR|Spawn")
